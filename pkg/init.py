...
def set_paths(config_path):
    ...
    for directory in data_directory_structure:
        ...
        if not os.path.exists(new_dir):
            os.makedirs(new_dir)
        with open(os.path.join(config_path, 'pkg_config.yml'), 'a') as c:
...
def installer_script():
    config_path = os.path.join(cwd, 'pkg/config/')
    with open(os.path.join(config_path, 'pkg_config.yml'), 'w') as c:
        c.write('version: '+version+'\n')
        c.write('pkg_directory: "'+os.path.join(cwd, 'pkg').replace('\\', '/')+'"\n')
