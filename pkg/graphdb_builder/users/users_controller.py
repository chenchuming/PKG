...
def parseUsersFile(expiration=365):
    ...
    """
    usersDir = pkg_config['users_directory']
    usersFile = os.path.join(usersDir, config['usersFile'])
    usersImportDir = pkg_config['imports_users_directory']
...
def get_user_creation_queries():
    ...
    try:
        ...
        user_creation_cypher = pkg_utils.get_queries(os.path.join(cwd, queries_path))
    except Exception as err:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
