...
def reset_mapping(entity):
    ...
    if entity in dbconfig["sources"]:
        directory = os.path.join(pkg_config["databases_directory"], dbconfig["sources"][entity])
...
def getMappingFromOntology(ontology, source=None):
    ...
    dirFile = os.path.join(pkg_config["ontologies_directory"], ont)
...
def getMappingForEntity(entity):
    ...
    mapping = {}
    if entity in dbconfig["sources"]:
        mapping_file = os.path.join(pkg_config["databases_directory"], os.path.join(dbconfig["sources"][entity], "complete_mapping.tsv"))
        max_wait = 0
        while not os.path.isfile(mapping_file) and max_wait < 5000:
            time.sleep(15)
            max_wait += 1
...
def getSTRINGMapping(source="BLAST_UniProt_AC", download=True, db="STRING"):
    ...
    mapping = defaultdict(set)
    directory = os.path.join(pkg_config["databases_directory"], db)
