...
from pkg.graphdb_connector import connector
import zipfile
import rarfile
...
def convert_pkg_clinical_to_sdrf(df):
    out_mapping = {'tissue':'characteristics[organism part]',
                   'disease': 'characteristics[disease]',
                   'grouping1': 'characteristics[phenotype]',
                   'analytical_sample': 'comment[data file]',
...
def get_config(config_name, data_type='databases'):
    ...
    """
    directory = os.path.join(pkg_utils.read_pkg_config(key='pkg_directory'), 'graphdb_builder')
    config = pkg_utils.get_configuration(os.path.join(directory, '{}/config/{}'.format(data_type, config_name)))
...
def setup_config(data_type="databases"):
    ...
    """
    try:
        dirname = os.path.join(pkg_utils.read_pkg_config(key='pkg_directory'), 'graphdb_builder')
        file_name = '{}/{}_config.yml'.format(data_type, data_type)
        config = pkg_utils.get_configuration(os.path.join(dirname, file_name))
    except Exception as err:
...
def downloadDB(databaseURL, directory=None, file_name=None, user="", password="", avoid_wget=False):
    ...
    if directory is None:
        directory = pkg_utils.read_pkg_config()["databases_directory"]
