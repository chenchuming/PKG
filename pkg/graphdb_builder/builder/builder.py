...
import argparse
import logging
from pkg.graphdb_builder.builder import importer, loader
from pkg import pkg_utils
from pkg.graphdb_builder import builder_utils

try:
    pkg_config = pkg_utils.read_pkg_config()
    log_config = pkg_config['graphdb_builder_log']
...
def run_minimal_update(user, n_jobs=3):
    ...
    minimal_load = ['ontologies', 'modified_proteins', 'drugs', 'mentions',
                    'side effects', 'clinical_variants', 'project', 'experiment']
    logger.info(
        "The user {} chose to perform a minimal build, after creating the database from a dump".format(user))
    logger.info(
        "Building database > step 1: Importing licensed ontologies and databases")
    ...
    importer.databasesImport(databases=licensed_dbs,
                             n_jobs=n_jobs, download=False)
    logger.info(
...
def run_full_update(user='pkg', download=True, n_jobs=3):
    logger.info("The user {} chose to perform a full build".format(user))
    logger.info(
        "Building database > step 1: Importing data from ontologies, databases and experiments")
    importer.fullImport(download=download, n_jobs=n_jobs)
    logger.info(
        "Building database > step 2: Loading all data imported into the database")
    loader.fullUpdate()

    return True


def update_textmining(user='pkg', download=True, n_jobs=3):
    logger.info(
        "The user {} chose to perform an update of the text mining".format(user))
    logger.info("Updating text mining > step 1: Importing data from mentions")
    importer.databasesImport(
        databases=['mentions'], n_jobs=n_jobs, download=download)
    logger.info(
        "Updating text mining > step 2: Loading updated mentions into the database")
...
def set_arguments():
    ...
    parser.add_argument("-b", "--build_type", help="define the type of build you want (import, load, full or minimal (after dump file))",
                        type=str, choices=['import', 'load', 'full', 'minimal'], default='full')
    parser.add_argument("-i", "--import_types", help="If only import, define which data types (ontologies, experiments, databases, users) you want to import (partial import)",
                        nargs='+', default=None, choices=['experiments', 'databases', 'ontologies', 'users'])
    parser.add_argument("-l", "--load_entities",
                        help="If only load, define which entities you want to load into the database (partial load)",  nargs='+', default=config["graph"])
    parser.add_argument(
        "-d", "--data", help="If only import, define which ontology/ies, experiment/s or database/s you want to import",  nargs='+', default=None)
    parser.add_argument(
        "-s", "--specific", help="If only loading, define which ontology/ies, projects you want to load",  nargs='+', default=[])
    parser.add_argument(
        "-n", "--n_jobs", help="define number of cores used when importing data", type=int, default=4)
    parser.add_argument(
        "-w", "--download", help="define whether or not to download imported data", type=str, default="True")
    parser.add_argument(
        "-u", "--user", help="Specify a user name to keep track of who is building the database", type=str, required=False)
...
def main():
    ...
    if args.build_type == 'full':
            ...
            if args.data is None or len(args.data) > 0:
                ...
                for import_type in args.import_types:
                    ...
                    if import_type.lower() == 'experiments' or import_type.lower() == 'experiment':
                        importer.experimentsImport(
                            projects=args.data, n_jobs=1)
                        ...
                        if args.data is not None:
                            valid_entities = [
                                x.lower() for x in args.data if x.lower() in databases]
                        else:
                            valid_entities = databases
                        if len(valid_entities) > 0:
                            logger.info("These entities will be imported: {}".format(
                                ", ".join(valid_entities)))
                            print("These entities will be imported: {}".format(
                                ", ".join(valid_entities)))
                            importer.databasesImport(
                                databases=valid_entities, n_jobs=args.n_jobs, download=download)
                        else:
                            logger.error(
                                "The indicated entities (--data) cannot be imported: {}".format(args.data))
                            print(
                                "The indicated entities (--data) cannot be imported: {}".format(args.data))
                        ...
                        if args.data is not None:
                            valid_entities = [
                                x.capitalize() for x in args.data if x.lower() in ontologies]
                        else:
                            valid_entities = ontologies
                        if len(valid_entities) > 0:
                            logger.info("These entities will be imported: {}".format(
                                ", ".join(valid_entities)))
                            print("These entities will be loaded into the database: {}".format(
                                ", ".join(valid_entities)))
                            importer.ontologiesImport(
                                ontologies=valid_entities, download=download)
                        else:
                            logger.error(
                                "The indicated entities (--data) cannot be imported: {}".format(args.data))
                            print(
        ...
        logger.info("The build will load data into the database: {}".format(
            "".join(args.load_entities)))
        valid_entities = []
        specific = args.specific
        if len(args.load_entities) > 0:
            valid_entities = [
                x.lower() for x in args.load_entities if x.lower() in config['graph']]
        else:
            valid_entities = config['graph']
        if len(valid_entities) > 0:
            logger.info("These entities will be loaded into the database: {}".format(
                ", ".join(valid_entities)))
            print("These entities will be loaded into the database: {}".format(
                ", ".join(valid_entities)))
            ...
            logger.error(
                "The indicated entities (--load_entities) cannot be loaded: {}".format(args.load_entities))
