...
def updateDB(driver, imports=None, specific=[]):
    ...
    try:
        cypher_queries = pkg_utils.get_queries(os.path.join(cwd, config['cypher_queries_file']))
    except Exception as err:
    ...
    for i in imports:
        ...
        try:
            import_dir = quote(pkg_config['imports_databases_directory'], safe='/:')
            if i == "ontologies":
                entities = [e.lower() for e in config["ontology_entities"]]
                if len(specific) > 0:
                    entities = list(set(entities).intersection([s.lower() for s in specific]))
                import_dir = quote(pkg_config['imports_ontologies_directory'], safe='/:')
                ...
                import_dir = quote(pkg_config['imports_curated_directory'], safe='/:')
                ...
                usersDir = pkg_config['imports_users_directory']
                ...
                import_dir = quote(pkg_config['imports_experiments_directory'], safe='/:')
...
def archiveImportDirectory(archive_type="full"):
    ...
    """
    dest_folder = pkg_config["archive_directory"]
    builder_utils.checkDirectory(dest_folder)
    folder_to_backup = pkg_config["imports_directory"]
